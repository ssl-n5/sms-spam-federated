...
def stemmer(source):
	...
	global PERIOD_FLAG #to check if nasa start or end ng sentence
	global PASS_FLAG
    ...
    #no need to put mode, use tokens = source.split(' ')
	...
	for token in tokens:		
		...
		'''if (PERIOD_FLAG == True and token[0].isupper()) or \
			(PERIOD_FLAG == False and token[0].islower()): #hindi gagana if hindi uppercase first letter or if uppercase siya sa gitna ng sentence
		'''
		token 	 = token.lower()		
...
def check_validation(token):
	with open('validation.txt', 'r', encoding='latin-1', errors='ignore') as valid:
...
#mode = sys.argv[1] # 1: Text File // 2: Raw String, check if for string or raw file, always 2?
#source = sys.argv[2] # 1: .txt name // 2: raw string, always 2?
#info_dis = sys.argv[3] # 1: no info // 2: show info, 1
